...
if [[ -z "$FIRST_ITEM" ]]; then
  ...
  if [[ -n "$SAME_OPTION" ]]; then
    WRITE_ERR="$(gh project item-edit --id "$ITEM_ID" --project-id "$PROJECT_ID" \
      ...
      say ""
      say "    Fix: the fine-grained PAT needs Projects: Read and write, and that"
      say "    permission is offered under ORGANISATION permissions. The repository"
      say "    and the board must both be owned by an organisation — a personal repo"
      say "    can read a board and will never move a card."
      say ""
      say "      github.com/account/organizations/new  (free, one minute)"
      say "      then transfer the repo in, and create the board under the org"
      say ""
      say "    Not worth it? Set board.optional: true and run on labels alone. The"
      say "    loop behaves identically; you track progress on the issues list."
